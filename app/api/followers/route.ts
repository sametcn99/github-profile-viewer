...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const lastPageLink = followers.headers.link;
    ...
    if (page && parseInt(page, 10) < lastPageNumber + 1) {
      // Return the followers' data only if the current page is less than the last page
      return NextResponse.json(followers.data);
    } else {
      return NextResponse.json({
        error: "This is the last page of followers.",
      });
    }
  } catch (error: any) {
